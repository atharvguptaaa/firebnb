...
export default function PhotoUploader({addedPhotos,onChange}){
  ...
  const [photoLink, setPhotoLink] = useState("");
    ...
    async function addPhotoByLink(e){
      ...
      async function removePhoto(e,filename){
        e.preventDefault();
        try{  
        axios.delete(`/remove-photo/${filename}`)
      ...
      async function selectAsMainPhoto(e,filename){
        e.preventDefault();
        try{       
    ...
    {addedPhotos.length > 0 && addedPhotos.map(link => (
        <div className="h-36 flex w-full relative " key={link}>
            <img className="rounded-2xl h-full w-full object-cover" src={`http://localhost:4000/uploads/${link}`} alt="" />
            <button onClick={(e)=>{removePhoto(e,link)}} className="cursor-pointer absolute bottom-1 right-1 text-white bg-black p-2 bg-opacity-50 rounded-full">
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
            ...
            <button onClick={(e)=>{selectAsMainPhoto(e,link)}} className="cursor-pointer absolute bottom-1 left-1 text-white bg-black p-2 bg-opacity-50 rounded-full">
